    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    {
        var requestedEmail = forgotPasswordDto.Email.Trim().ToLowerInvariant();
        var user = await userRepository.GetByEmailAsync(requestedEmail);
        if (user == null)
        {
            ...
            return new EmailResponseDto
            {
                ...
                Data = new { email = requestedEmail, initiated = true }
            };
        }
        ...
        catch (Exception ex)
        {
            ...
            {
                Success = false,
                Message = "No se pudo enviar el correo de recuperaciÃ³n. Intenta nuevamente mÃ¡s tarde.",
                Data = new { email = user.Email, initiated = false }
            };
        }
